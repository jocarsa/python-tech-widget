...
for frame_number in range(total_frames):
    ...
    for circle in circles:
        ...
        if circle.t == 0:
            # Draw a simple arc (stroke only)
            cv2.ellipse(frame, center, (circle.r, circle.r), 0, 
                        np.degrees(circle.a1), np.degrees(circle.a2), 
                        (0, 0, 0), circle_width)
        elif circle.t == 1:
            # Draw a filled arc segment
            pts = np.array([[center[0], center[1]], 
                            [center[0] + int(circle.r * np.cos(circle.a1)), center[1] + int(circle.r * np.sin(circle.a1))], 
                            [center[0] + int(circle.r * np.cos(circle.a2)), center[1] + int(circle.r * np.sin(circle.a2))]], np.int32)
            cv2.fillPoly(frame, [pts], (0, 0, 0))
            cv2.ellipse(frame, center, (circle.r, circle.r), 0, 
                        np.degrees(circle.a1), np.degrees(circle.a2), 
                        (0, 0, 0), -1)
        elif circle.t == 2:
            # Draw a stroked arc with additional thick lines at the start and end
            ...
            # Draw a stroked arc with small filled circles at start and end points
            cv2.ellipse(frame, center, (circle.r, circle.r), 0, 
                        np.degrees(circle.a1), np.degrees(circle.a2), 
                        (0, 0, 0), 1)
            cv2.circle(frame, (center[0] + int(circle.r * np.cos(circle.a1)), 
                               center[1] + int(circle.r * np.sin(circle.a1))), 
                       3, (0, 0, 0), -1)
            cv2.circle(frame, (center[0] + int(circle.r * np.cos(circle.a2)), 
                               center[1] + int(circle.r * np.sin(circle.a2))), 
                       3, (0, 0, 0), -1)
