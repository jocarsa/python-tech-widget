...
for frame_number in range(total_frames):
    ...
    for circle in circles:
        ...
        if circle.t == 0:
            ...
            # Draw a filled arc instead of a polygon
            temp_frame = np.ones_like(frame) * 255  # Create a temporary white frame
            cv2.ellipse(temp_frame, (width//2, height//2), (circle.r, circle.r), 0, 
                        np.degrees(circle.a1), np.degrees(circle.a2), 
                        (0, 0, 0), -1)
            # Blend the temp frame with the original to simulate the filled arc
            frame = cv2.addWeighted(frame, 1, temp_frame, 0.3, 0)
        elif circle.t == 2:
